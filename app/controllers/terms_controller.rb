class TermsController < ApplicationController
  ...
  def search_word 
    search_term_id = params[:q]
    websters_term = Term.find_by(id: search_term_id, dictionary_id: 3)

    #t = Term.where(name: 'person').where("dictionary_id in (?)", [1, 2, 4, 5]).includes(:dictionary)    
    t = Term.where(name: 'person').includes(:dictionary)
    #t = Term.where(name: 'mind').includes(:dictionary).pluck( :name, :dictionary_id)

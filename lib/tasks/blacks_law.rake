namespace :BlacksLaw2 do
  ...
  #hanging falls rock state park 
  
  
def parse_site_content(url)
  dictionary = Dictionary.find_or_create_by(name: "Black's Law Dictionary 2nd Edition")
  doc = Nokogiri::HTML(open(url))  
  doc.css('.post').each do |link|
    puts link.content
  ...
  desc "Black law website  parse"
  task parse_blackslaw_site: :environment do
    url = 'http://thelawdictionary.org/letter/p/'  
    ('b'..'z').to_a.each do |letter| 
      parse_site_content("#{url}/letter/#{letter}/")
    end 
  end 
